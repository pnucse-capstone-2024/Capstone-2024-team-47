...
namespace NetworkCore.Packet
{
    public abstract class PacketSession : Session
    {
        ...
        public sealed override int OnRecv(ArraySegment<byte> buffer)
        {
            ...
            else
            {
                while (true)
                {
                    ...
                    Logger.DebugLog($"Encrypted Packet : {BitConverter.ToString(encryptedPacket).Replace("-", " ")}");
                    Logger.DebugLog($"Decrypted Packet : {BitConverter.ToString(decryptedPacket).Replace("-", " ")}");
        ...
        protected void StartSessionKeyExchange()
        {
            ...
            // CipherSuite cipherSuite = CipherSuite.AesGcm;
